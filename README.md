...
- **タスク（`- [ ]`）を含む Markdown は「ツリー」で表示**（`ツリー` / `プレビュー` / `編集` の 3 サブタブ）
  ...
  - タスクの下に字下げした `期日:` の行を**期日**として読む（`期日: 2026-09-21` / `期日: 2026-09-21 06:35` /
    `期日: 2026-09-21 12:45〜13:05` / 時刻だけの `期日: 06:35` は親の期日の日付を借りる。`予定:`・`日時:`・`due:`・`when:` も同じ。
    後ろの文字〔`PDT` など〕は添え書きとして出すだけで、時差は計算しない）。読めない書き方はふつうのメモのまま残る
...
- **Tasks タブで `TODO.md` のタスクを選び、このプロジェクトで動いている Claude Code のセッションへ渡して実行**
  ...
  - **ボタンの上の「追加の指示（任意）」**にその場の条件を書ける（例: 「Phase 6 だけやって」）。書いた文面は
    実行 / プラン作成 / 説明の文面の末尾に「追加の指示」として足され、食い違ったときはそちらを優先させる。
    **空欄なら今までと同じ文面**（削除には効かない）。Ctrl+Enter で実行。打ちかけはタスクごとに覚えるので、TODO.md が外で変わって画面を描き直しても消えない
  - **左ペインは `ツリー` ｜ `タイムライン` を切り替えられる**（期日のあるタスクが 1 件でもあるときだけ出る。選んだ方を覚える）。
    タイムラインは期日のあるタスクを日ごとの見出し → 時刻の順に並べる（時刻の無い行は書かれた場所に出る）。済んだタスクはツリーと同じく並べず
    （全部済んだ日は見出しごと出ない）、`済みも出す` を入れたときだけ薄く出す（選んだ方を覚える。見出しの `済 / 全部` の数はどちらでも出る）。
    今日の見出しには「今日」、その中に「いま」の線、期日を過ぎて済んでいない行には「過ぎた」の印（ブラウザの時計と比べる。1 分おきに描き直す）。
    日の見出しを押すと右ペインに**その日の一覧**（`#tasks/@day/<日付>`。札は全文）。行や札を押した後は、ツリーから選んだときと同じタスクの詳細
    （実行 / プラン作成 / 説明）。タスクの詳細には期日のチップが出る
  ...
  - 文面はサーバが TODO.md から組む（ブラウザからは id と決め打ちの種別、それに「追加の指示」の本文しか受けない）。
...
- **Files タブでプロジェクト内のファイルをすべて編集**（テキストエディタと同じ扱い）
  ...
  - **dotfile も出す**（`.env` を含む）。除外は既定で `.git/` と `node_modules/` だけ
    （`files.exclude` で足せる。既定の 2 つは外せない）
...
```jsonc
{
  ...
  "files": {
    "label": "Files",                      // タブの表示名
    // ツリーからも読み書き（開く・保存・名前の変更・削除）からも外す**ディレクトリ / ファイル名**。
    // パス区切りやグロブは受けない（パスのどこかのセグメントが一致したら除外）。
    // **既定の .git / node_modules に足す**（書き写さなくてよい）。既定は外せない
    // （除外は読み書きの防壁でもあるので、書き忘れで .git が画面から編集できるようにしない）
    "exclude": ["dist"]
  },
...
- 同じ OS ユーザーなら誰でもその受信口に書ける。vibeboard は `127.0.0.1` 固定で、`register` はループバックからしか受けず、
  文面はサーバが TODO.md から組む。ブラウザから足せるのは画面の**「追加の指示」だけ**で、制御文字を落として 4000 字までに絞ってから
  組んだ文面の末尾に付ける（`src/todo.ts` の `sanitizeNote` / `appendNote`）
